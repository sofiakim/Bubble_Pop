...
/** The menu screen for Bubble Pop board
  * @author: Sofia Kim, Jessie Ma, Tracy Lei
  * @version: June 2012
 */
public class MenuScreen extends JFrame implements ActionListener
{
    ...
    private Image imageBackground, imageNewGame, imageNewGameBlack, imageHelp, imageHelpBlack,
	imageRank, imageRankBlack, imageAbout, imageAboutBlack, imageExit, imageExitBlack;
    ...
    private BoardFrame graphicBoard;



    /** Constructs a menu screen frame and sets up the game to start.
      */
    public MenuScreen ()
    {
	...
	imageBackground = new ImageIcon ("project images/BubblePopStartMenu.png").getImage ();
	//imageHelp1 = new ImageIcon ("Images/HelpInstructions1.png").getImage ();
	imageNewGame = new ImageIcon ("project images/new game.png").getImage ();
	imageNewGameBlack = new ImageIcon ("project images/new game black.png").getImage ();
	imageHelp = new ImageIcon ("project images/help.png").getImage ();
	imageHelpBlack = new ImageIcon ("project images/help black.png").getImage ();
	imageRank = new ImageIcon ("project images/rank.png").getImage ();
	imageRankBlack = new ImageIcon ("project images/rank black.png").getImage ();
	imageAbout = new ImageIcon ("project images/about.png").getImage ();
	imageAboutBlack = new ImageIcon ("project images/about black.png").getImage ();
	imageExit = new ImageIcon ("project images/exit.png").getImage ();
	imageExitBlack = new ImageIcon ("project images/exit black.png").getImage ();


	// set size of the drawing panel to accommodate your images (in this case the clouds and the side images)
	Dimension imageSize = new Dimension (900, 600);
	...
	public void paintComponent (Graphics g)
	{
		...
		g.drawImage (imageNewGameBlack, 725, 150, 150, 60, this);
	    else
		g.drawImage (imageNewGame, 725, 150, 150, 60, this);

	    if (highlightHelp)
		// Images can be scaled to size, such as in the line below (70 x 40 pixels), but scaling slows down things.
		g.drawImage (imageHelpBlack, 725, 230, 150, 60, this);
	    else
		g.drawImage (imageHelp, 725, 230, 150, 60, this);

	    if (highlightRank)
		// Images can be scaled to size, such as in the line below (70 x 40 pixels), but scaling slows down things.
		g.drawImage (imageRankBlack, 725, 310, 150, 60, this);
	    else
		g.drawImage (imageRank, 725, 310, 150, 60, this);

	    if (highlightAbout)
		// Images can be scaled to size, such as in the line below (70 x 40 pixels), but scaling slows down things.
		g.drawImage (imageAboutBlack, 725, 390, 150, 60, this);
	    else
		g.drawImage (imageAbout, 725, 390, 150, 60, this);

	    if (highlightExit)
		// Images can be scaled to size, such as in the line below (70 x 40 pixels), but scaling slows down things.
		g.drawImage (imageExitBlack, 725, 470, 150, 60, this);
	    else
		g.drawImage (imageExit, 725, 470, 150, 60, this);
	...
	public void mousePressed (MouseEvent event)
	{
	    ...
	    if (pressed.x >= 725 && pressed.x < 875 && pressed.y >= 470 && pressed.y < 530)
	    {
		dispose ();
		System.exit (0);
	    }
	    ...
	    if (pressed.x >= 725 && pressed.x < 875 && pressed.y >= 150 && pressed.y < 210)
	    {
		gameBoard.newGame ();
	    }
	...
	*/
	public void mouseMoved (MouseEvent event)
	{
	    ...
	    if (pos.x >= 725 && pos.x < 875 && pos.y >= 150 && pos.y < 210) //&& helpScreen == false)
	    {
	    ...
	    if (pos.x >= 725 && pos.x < 875 && pos.y >= 230 && pos.y < 290) // && helpScreen == false)
	    {
		highlightHelp = true;
		setCursor (Cursor.getPredefinedCursor (Cursor.HAND_CURSOR)); // change mouse cursor to a hand
	    }
	    ...
	    if (pos.x >= 725 && pos.x < 875 && pos.y >= 310 && pos.y < 370) // && helpScreen == false)
		highlightRank = true;
	    else
		highlightRank = false;
	    ...
	    if (pos.x >= 725 && pos.x < 875 && pos.y >= 390 && pos.y < 450) // && helpScreen == false)
		highlightAbout = true;
	    else
	    ...
	    if (pos.x >= 725 && pos.x < 875 && pos.y >= 470 && pos.y < 530) // && helpScreen == false)
		highlightExit = true;
	    else
		highlightExit = false;
    ...
    {
	MenuScreen demo = new MenuScreen ();
